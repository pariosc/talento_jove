...
@router.post("/")
async def crear_oferta(oferta: Oferta, conn = Depends(get_conexion)):
    consulta = """
        INSERT INTO "OFERTAS"("PK_id_oferta", "FK_id_empresa", "titulo", 
                             "descripcion", "requisitos", "fecha_limite", "estado") 
        VALUES(%s, %s, %s, %s, %s, %s, %s)
    """
    parametros = (oferta.PK_id_oferta, oferta.FK_id_empresa, oferta.titulo, 
                  oferta.descripcion, oferta.requisitos, oferta.fecha_limite, oferta.estado)
    try:
        async with conn.cursor() as cursor:
            await cursor.execute(consulta, parametros)
            await conn.commit()
            return {"mensaje": "Oferta laboral publicada exitosamente"}
    except Exception as e:
